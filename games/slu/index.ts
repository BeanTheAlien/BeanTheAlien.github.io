...
var cps = 0;
var cpc = 1;
function upgr(item: keyof typeof costMap, k: KeyedTextUI<number>, mo: "s" | "c", next: number) {
    k.val = costMap[item];
    return () => {
...
function upg(item: keyof typeof costMap, xOff: number, y: number, c1t: string, mo: "s" | "c", next: number) {
    const u = new ButtonUI({ scene, styles: globalBS, w: bw / 1.45, h: bw / 4, x: upgBox.x + bw / 6, y });
    const kui = new KeyedTextUI<number>({ scene, change: () => `${costMap[item]} / +${next} Mp${mo}`, x: u.width / 9, y: u.height - u.height / 5 });
