import { DebugRay, Entity, objIs, PlayableCharacter, Scene, Vector, BulletObject, Angle } from "../../phantom2d.js";
...
scene.add(plr);
scene.on("click", () => {
    const o = new BulletObject({ x: plr.x, y: plr.y, rot: scene.rotToMouse(plr), height: 6, width: 18, scene, color: "#e2e603", collide: (e) => { if (objIs(e, Enemy)) {
            e.comp("health").hurt(stat.dmg);
            scene.rm(o);
        } }, extLeft: 0, extRight: scene.width, extTop: 0, extBtm: scene.height, spd: stat.bspd });
    scene.add(o);
});
scene.start(() => {
    scene.bg("#003764");
