import { Angle, ButtonUI, chance, Entity, KeyedTextUI, random, Scene, SceneUI, StaticObject, TextUI } from "../../../phantom2d.js";
...
scene.on("click", () => {
    ...
    next();
    if(multi > 0 && chance(multi * 10, 1000)) for(let i = 0; i < multiCn; i++) next();
});
scene.add(nk, tr);
var gdy = 50;
function disp(x: string, v: number) {
    const j = new KeyedTextUI<number>({ scene, change: (v) => `${x}: ${v}`, y: gdy, x: 15 });
    j.val = v;
    gdy += 50;
    return j;
}
...
function upg(item: keyof typeof costMap, xOff: number, y: number, c1t: string, mo: Mop, lbl: string, next: number) {
    const u = new ButtonUI({ scene, styles: globalBS, w: bw / 1.45, h: bw / 4, x: upgBox.x + bw / 6, y });
    const kui = new KeyedTextUI<number>({ scene, change: () => `${costMap[item]} / +${next} ${lbl}`, x: u.width / 9, y: u.height - u.height / 5 });
