...
class Scene {
     ...
     */
    static unloadListenerCreated = false;
    canvas;
    ctx;
    items;
    evStore;
    ...
    findByTag(tagName) {
        return this.find((e) => this.#tagTest(e, tagName));
    }
    hasByTag(tagName) {
...
class Raycast extends RaycastBase {
    cast() {
        let res = null;
        super.cast((i, hit, dir) => {
            if ((!this.self || i != this.self) && (res == null || hit < res.dist))
...
class External {
    ...
    open(url, target, features) {
        const _url = url ?? "about:blank";
        const _tg = FinalizeOpeningMode(target ?? "_blank");
